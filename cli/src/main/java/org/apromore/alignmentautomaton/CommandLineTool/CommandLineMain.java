package org.apromore.alignmentautomaton.CommandLineTool;
...
import org.apromore.alignmentautomaton.ConformanceWrapperTR;
import org.apromore.alignmentautomaton.ConformanceWrapperTRMT;
import org.apromore.alignmentautomaton.FitnessWithTRreductionMain;
...
public class CommandLineMain {
  ...
  public static void main(String... args) {
    try {
      ...
      if (args.length < 3) {
        ...
        System.out.println("Extension: " + confTask.type + ", Model: " + confTask.model + ", Time: " + confTask.time
            + " ms, Avg. Raw fitness cost: " + df2.format(confTask.cost) + ", Fitness: " + df2
            .format(confTask.fitness));
  ...
  public static void printAlignments(String path, PNMatchInstancesRepResult alignmentResult,
      UnifiedMap<Integer, String> caseIDs) {
    ...
    String[] headlines = { PNMatchInstancesRepResult.ORIGTRACELENGTH, PNMatchInstancesRepResult.TIME,
        PNMatchInstancesRepResult.RAWFITNESSCOST, PNMatchInstancesRepResult.TRACEFITNESS };
    for (int i = 0; i < headlines.length; i++) {
      cell = row.createCell(i + 1);
      cell.setCellValue(headlines[i]);
    }
    row = caseTypeSheet.createRow(3);
    String[] results = { alignmentResult.getInfo().get(PNMatchInstancesRepResult.ORIGTRACELENGTH),
    ...
    headlines = new String[] { "Case Type", "#Represented cases", PNMatchInstancesRepResult.ORIGTRACELENGTH,
        PNMatchInstancesRepResult.TIME, PNMatchInstancesRepResult.RAWFITNESSCOST,
        PNMatchInstancesRepResult.TRACEFITNESS, "alignment" };
    for (int i = 0; i < headlines.length; i++) {
      cell = row.createCell(i + 1);
      cell.setCellValue(headlines[i]);
    }
    ...
    headlines = new String[] { "Num.", "Case Type", "Case ID", "Trace Index", PNMatchInstancesRepResult.ORIGTRACELENGTH,
        PNMatchInstancesRepResult.TIME, PNMatchInstancesRepResult.RAWFITNESSCOST,
        PNMatchInstancesRepResult.TRACEFITNESS, "Alignment" };
    for (int i = 0; i < headlines.length; i++) {
      cell = row.createCell(i + 1);
    ...
    for (AllSyncReplayResult res : alignmentResult) {
      ...
      for (int trace : res.getTraceIndex()) {
        ...
        results = new String[] { String.valueOf(num++), String.valueOf(caseType), caseIDs.get(trace),
            String.valueOf(trace), String.valueOf(res.getInfo().get(PNMatchInstancesRepResult.ORIGTRACELENGTH)),
            ...
            String.valueOf(res.getInfo().get(PNMatchInstancesRepResult.TRACEFITNESS)), alignment };
        for (int i = 0; i < results.length; i++) {
          cell = row.createCell(i + 1);
          cell.setCellValue(results[i]);
        }
      ...
      results = new String[] { String.valueOf(caseType), String.valueOf(res.getTraceIndex().size()),
