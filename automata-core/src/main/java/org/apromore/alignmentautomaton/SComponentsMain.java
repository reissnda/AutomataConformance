...
//        result =
//            args[3] + "," + args[0] + "," + args[1] + "," + "," + "," + "," + args[2] + "," + ", time out" + "," + ","
//                + "\n";
//        future.cancel(true);
//      }
...
//        }
//        executor.shutdown();
//        if (numExecutions >= 3 && (i == 0 || i == numExecutions - 1)) {
//          continue;
//        }
...
//      }
//      result = confTask.type + "," + confTask.model + "," + confTask.nSComps + "," + confTask.nParallel + ","
//          + confTask.nChoice + "," + confTask.log + "," + confTask.logSize + "," + times.average() + "," + costs
//          .average() + "," + conflicts.average() + "\n";
//      System.out.println(result);
//      recordResult(args[0], result);
//      System.exit(0);
//    }
//  }
  ...
  private static void recordStatistics(String path, int dataset, DecomposingConformanceImporter stats)
      throws Exception {
    FileWriter pw = null;
    ...
    if (!statF.exists()) {
      ...
      pw.append(
          "Path,Model,#Places,#Transitions,#Arcs,Size,#Choices,#Parallel,#RGNodes,#RGArcs,RGSize,#SComponents,@Places,&Places,@Transitions,&Transitions,@Arcs,&Arcs,@Size,&Size,@RGNodes,&RGNodes,@RGArcs,&RGArcs,@RGSize,&RGSize,@Choices,&Choices\n");
    }
    if (pw == null) {
      pw = new FileWriter(path + "stats.txt", true);
    }
    String result =
        path + "," + dataset + "," + stats.places + "," + stats.transitions + "," + stats.arcs + "," + stats.size + ","
            + stats.choice + "," + stats.parallel + "," + stats.rg_nodes + "," + stats.rg_arcs + "," + stats.rg_size
            + ",";
    if (stats.doDecomposition) {
      result += stats.sComponentFSMs.size() + "," + stats.scompPlaces.average() + "," + stats.scompPlaces.sum() + ","
          + stats.scompTransitions.average() + "," + stats.scompTransitions.sum() + "," + stats.scompArcs.average()
          + "," + stats.scompArcs.sum() + "," + stats.scompSize.average() + "," + stats.scompSize.sum() + ","
          + stats.scompRGNodes.average() + "," + stats.scompRGNodes.sum() + "," + stats.scompRGArcs.average() + ","
          + stats.scompRGArcs.sum() + "," + stats.scompRGSize.average() + "," + stats.scompRGSize.sum() + ","
          + stats.scompChoices.average() + "," + stats.scompChoices.sum() + "\n";
    } else {
      result += "1\n";
    }
    pw.append(result);
