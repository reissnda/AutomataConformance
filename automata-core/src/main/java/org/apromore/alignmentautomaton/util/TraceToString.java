...
public class TraceToString {
  ...
  public static String convertXTraceToString(XTrace trace, NameExtractor nameExtractor, String skipLabel,
  ...
  public static String convertXTraceToString(XTrace trace, NameExtractor nameExtractor, String skipLabel,
      Map<String, Set<String>> parallel) {
    List<String> labels = new ArrayList<>(trace.size());
    for (int i = 0; i < trace.size(); i++) {
      String name = nameExtractor.getEventName(trace.get(i));
      if (!name.equals(skipLabel)) {
  ...
  public static String[] convertXTraceToListOfString(XTrace trace, Map<String, String> nameMap,
  ...
  public static String convertXTraceToStringWithTimestamp(XTrace trace, NameExtractor nameExtractor,
      SimpleDateFormat simpleDateFormat) {
    List<String> labels = new ArrayList<>(trace.size());
    for (int i = 0; i < trace.size(); i++) {
      labels
          .add(nameExtractor.getEventName(trace.get(i)) + simpleDateFormat.format(xte.extractTimestamp(trace.get(i))));
    }
    return listToString(labels);
  }
