...
public class LogModifier {
  ...
  private static boolean checkViolation(XTrace trace) {
    ...
    for (int i = 0; i < trace.size(); i++) {
      ...
      if ("Leucocytes".equalsIgnoreCase(current_name)) {
        ...
        if ("IV Liquid".equalsIgnoreCase(current_name) || "IV Antibiotics".equalsIgnoreCase(current_name)
            || "Admission IC".equalsIgnoreCase(current_name) || "Admission NC".equalsIgnoreCase(current_name)) {
          treated = true;
        }
      }
    }
  ...
  private final Comparator<XTrace> comparatorXTrace = new Comparator<XTrace>() {
    @Override
