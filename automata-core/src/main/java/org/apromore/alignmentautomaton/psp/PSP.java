...
public class PSP {
  ...
  private final BiMap<Integer, Node> nodes;
  ...
  public PSP(BiMap<Integer, Node> nodes, Set<Arc> arcs, int sourceNode, Automaton logAutomaton,
  ...
  public PSP(BiMap<Integer, Node> nodes, Set<Arc> arcs, int sourceNode, Automaton logAutomaton,
      List<Automaton> sComponentAutomata) {
    this.nodes = nodes;
    this.arcs = arcs;
    this.sourceNode = sourceNode;
    this.finalNodes = new UnifiedSet<Node>();
  ...
  public List<Automaton> sComponentAutomata() {
    return this.sComponentAutomata;
  }
  ...
  public String arcLabel(Arc arc) {
    if (arc.transition().operation() == Configuration.Operation.RHIDE) {
      return "< " + arc.transition().operation() + " : " + this.modelAutomaton().eventLabels()
          .get(arc.transition().eventModel()) + " >";
    }
    return "< " + arc.transition().operation() + " : " + this.logAutomaton().eventLabels()
        .get(arc.transition().eventLog()) + " >";
  }
  ...
  public String nodeLabel(int nodeID) {
    ...
    if (nodeLabel.startsWith("{ ", nodeLabel.length() - 2)) {
