...
public class Node {
  ...
  public Node(State stLog, State stModel, Configuration configuration, int tracePosition, double weight, int posPenalty,
      boolean penalty) {
    this.stLog = stLog;
    this.stModel = stModel;
    this.stateLogID = stLog.id();
  ...
  private void setStandardCost() {
    ...
    for (Synchronization sync : configuration.sequenceSynchronizations()) {
      if ((sync.operation() == Configuration.Operation.RHIDE || sync.operation() == Configuration.Operation.LHIDE)
          && sync.eventModel() != Automaton.skipEvent) {
        currentCost += 1;
      }
    }
  ...
  public IntObjectHashMap<State> stSComps() {
    return this.stSComps;
  }
