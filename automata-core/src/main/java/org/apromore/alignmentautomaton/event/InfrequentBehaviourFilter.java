...
import com.raffaeleconforti.util.LogModifier;
import com.raffaeleconforti.util.LogOptimizer;
import org.apromore.alignmentautomaton.event.infrequentbehaviour.automaton.AutomatonInfrequentBehaviourDetector;
import org.apromore.alignmentautomaton.event.infrequentbehaviour.automaton.AutomatonInfrequentBehaviourRemover;
import org.apromore.alignmentautomaton.event.selection.NoiseFilterResult;
...
public class InfrequentBehaviourFilter {
  ...
  private final Percentile percentileC = new Percentile();
  ...
  private AutomatonInfrequentBehaviourDetector automatonInfrequentBehaviourDetector = new AutomatonInfrequentBehaviourDetector(
      AutomatonInfrequentBehaviourDetector.MAX);
  ...
  public InfrequentBehaviourFilter(XEventClassifier xEventClassifier, boolean useGurobi, boolean useArcsFrequency,
      boolean debug_mode, boolean removeTraces, boolean removeNodes, double percentile, double thresholdLimit,
      boolean deviance, double inputNoiseThreshold) {
  ...
  public XLog filterLog(final UIPluginContext context, XLog rawlog, NoiseFilterResult result) {
    ...
    if (removeNodes) {
      result.setRequiredStates(new HashSet<>());
    }

    LogModifier logModifier = new LogModifier(new XFactoryNaiveImpl(), XConceptExtension.instance(),
        XTimeExtension.instance(), logOptimizer);
    ...
    do {
      events = newEvents;
      XLog log3 = filter(context, log, result.getRequiredStates(), result.isFixLevel(), result.getNoiseLevel(),
          removeTraces, result.getPercentile());
  ...
  private XLog filter(final UIPluginContext context, XLog rawlog, Set<Node<String>> requiredStates, boolean isFixLevel,
      double noiseLevel, boolean excludeTraces, double percentile) {
    ...
    LogModifier logModifier = new LogModifier(factory, XConceptExtension.instance(), XTimeExtension.instance(),
        logOptimizer);
    logModifier.insertArtificialStartAndEndEvent(log);

    if (removeNodes) {
      requiredStates = new HashSet<>();
    }
    ...
    if (isFixLevel) {
      try {
        ...
        log2 = AutomatonInfrequentBehaviourRemover
            .removeInfrequentBehaviour(context, xEventClassifier, log, automaton, lowerbound, upperbound, -1,
                excludeTraces, deviance);
      ...
      if (lastAutomaton == null || !automaton.equals(lastAutomaton)) {
        try {
          finalLog = AutomatonInfrequentBehaviourRemover
              .removeInfrequentBehaviour(context, xEventClassifier, log, automaton, lowerbound, upperbound, 0.5,
                  excludeTraces, deviance);
          finalThreshold = noiseThreshold;
        } catch (HighThresholdException e) {
          System.out.println("Identifying best upperbound...");
          noiseThreshold = roundNumber(
              findBestUpperbound(context, log, requiredStates, upperbound / 2, upperbound, excludeTraces));
      ...
      if (finalThreshold == noiseThreshold && finalLog != null) {
        ...
        try {
          automaton = getFilteredAutomaton(automatonOriginal, requiredStates, noiseThreshold);
          log2 = AutomatonInfrequentBehaviourRemover
              .removeInfrequentBehaviour(context, xEventClassifier, log, automaton, 0, noiseThreshold, 0.5,
                  excludeTraces, deviance);
        } catch (HighThresholdException hte) {
          log.clear();
          return log;
  ...
  public Automaton<String> getFilteredAutomaton(Automaton<String> automatonOriginal, Set<Node<String>> requiredStates,
      double threshold) {
    Automaton<String> automaton = (Automaton<String>) automatonOriginal.clone();
    return automatonInfrequentBehaviourDetector
        .removeInfrequentBehaviour(automaton, requiredStates, threshold, useGurobi, useArcsFrequency);
  }

  private double findBestUpperbound(PluginContext context, XLog log, Set<Node<String>> requiredStates,
      double upperbound, double oldUpperbound, boolean excludeTraces) {
        ...
        if (upperbound != oldUpperbound) {
          ...
          if (lastAutomaton == null || !automaton.equals(lastAutomaton)) {
            if (!upperbounds.containsKey(noiseThreshold)) {
              ...
              finalLog = AutomatonInfrequentBehaviourRemover
                  .removeInfrequentBehaviour(context, xEventClassifier, log, automaton, upperbound, oldUpperbound, 0.5,
                      excludeTraces, deviance);
              finalThreshold = noiseThreshold;
  ...
  public XLog filterLog(XLog rawlog) {
    ...
    do {
      ...
      if (lastAutomaton == null || !automaton.equals(lastAutomaton)) {
        try {
          finalLog = AutomatonInfrequentBehaviourRemover
              .removeInfrequentBehaviour(context, xEventClassifier, log2, automaton, lowerbound, upperbound, 0.5, true,
                  false);
          finalThreshold = noiseThreshold;
        } catch (HighThresholdException e) {
          noiseThreshold = roundNumber(
              findBestUpperbound(context, log2, requiredStates, upperbound / 2, upperbound, true));
        }
      }
      ...
      if (finalThreshold == noiseThreshold && finalLog != null) {
        ...
        try {
          automaton = getFilteredAutomaton(automatonOriginal, requiredStates, noiseThreshold);
          log2 = AutomatonInfrequentBehaviourRemover
              .removeInfrequentBehaviour(context, xEventClassifier, log2, automaton, 0, noiseThreshold, 0.5, true,
                  false);
        } catch (HighThresholdException hte) {
          return log2;
        }
  ...
  public XLog filterDeviances(XLog rawlog) {
    ...
    if (inputNoiseThreshold > 0) {
      noiseThreshold = inputNoiseThreshold;
    } else {
      noiseThreshold = discoverThreshold(arcs, percentile);
    }
    ...
    automaton = getFilteredAutomaton(automatonOriginal, requiredStates, noiseThreshold);
    try {
      log = AutomatonInfrequentBehaviourRemover
          .removeInfrequentBehaviour(context, xEventClassifier, log, automaton, 0, noiseThreshold, 0, true, deviance);
    } catch (HighThresholdException e) {
      e.printStackTrace();
    }
