...
package org.apromore.alignmentautomaton.event.optimization.wrapper;
...
import org.apromore.alignmentautomaton.ilpsolverwrapper.ILPSolverExpression;
...
public class WrapperInfrequentBehaviourSolver<T> {
  ...
  public WrapperInfrequentBehaviourSolver(Automaton<T> automaton, Set<Edge<T>> infrequentEdges,
      Set<Node<T>> requiredStates, boolean useArcsFrequency) {
  ...
  public Set<Edge<T>> identifyRemovableEdges(ILPSolver solver) {
    ...
    for (int i = 0; i < edges.length; i++) {
      edges[i] = solver.addVariable(0.0, 1.0, 1.0, ILPSolver.VariableType.BINARY,
          edgeList.get(i).toString().replaceAll("-", "_").replaceAll(" ", ""));
    }
    ...
    for (int i = 0; i < connectedSourceList.length; i++) {
      connectedSourceList[i] = solver.addVariable(0.0, 1.0, 1.0, ILPSolver.VariableType.BINARY,
          "S_" + nodeList.get(i).toString().replaceAll("-", "_").replaceAll(" ", ""));
      connectedTargetList[i] = solver.addVariable(0.0, 1.0, 1.0, ILPSolver.VariableType.BINARY,
          "T_" + nodeList.get(i).toString().replaceAll("-", "_").replaceAll(" ", ""));
    }
        ...
        if (edge.getSource().equals(nodeList.get(i))) {
          for (int j = 0; j < nodeList.size(); j++) {
            if (i != j && edge.getTarget().equals(nodeList.get(j))) {
              subconnectedSourceList[i][j] = solver
                  .addVariable(0.0, solver.getInfinity(), 1.0, ILPSolver.VariableType.INTEGER,
                      "SL_" + nodeList.get(j).toString() + "_" + nodeList.get(i).toString().replaceAll("-", "_")
                          .replaceAll(" ", ""));
              subconnectedTargetList[i][j] = solver
                  .addVariable(0.0, solver.getInfinity(), 1.0, ILPSolver.VariableType.INTEGER,
                      "TL_" + nodeList.get(i).toString() + "_" + nodeList.get(j).toString().replaceAll("-", "_")
                          .replaceAll(" ", ""));
              break;
            }
          }
        ...
        for (int j = 0; j < nodeList.size(); j++) {
          if (i != j && subconnectedSourceList[i][j] != null) {
            for (int k = 0; k < edgeList.size(); k++) {
              if (edgeList.get(k).getSource().equals(nodeList.get(i)) && edgeList.get(k).getTarget()
                  .equals(nodeList.get(j))) {
    ...
    if (status == ILPSolver.Status.OPTIMAL) {
      if (debug_mode) {
        System.out.println("The optimal objective is " + solver.getSolutionValue());
      }
