...
public class DecomposingConformanceChecker {
  ...
  public DecomposingConformanceChecker(DecomposingConformanceImporter decompositions) throws IOException {
    ...
    if (decompositions.doDecomposition) {
      ...
      componentAlignments
          .put(0, new ScalableConformanceChecker(decompositions.dafsa, decompositions.modelFSM, Integer.MAX_VALUE));
  ...
  public void checkConformanceForComponents() {
    ...
    //parallelize here!
    this.minModelMoves = (int) Math.round(
        (new SingleTraceConformanceChecker(new IntArrayList(), decompositions.modelFSM,
    ...
    compList.parallelStream().forEach(sComp -> componentAlignments.put(sComp,
        new ScalableConformanceChecker(decompositions.componentDAFSAs.get(sComp),
            decompositions.sComponentFSMs.get(sComp), Integer.MAX_VALUE)));
		/*for(int pos=0; pos < decompositions.sComponentFSMs.size(); pos++)
		{
  ...
  public void recomposeConformance() throws IOException {
    ...
    if (conflict > 0) {
      ...
      PNMatchInstancesRepResult rescueRes = new ScalableConformanceChecker(
