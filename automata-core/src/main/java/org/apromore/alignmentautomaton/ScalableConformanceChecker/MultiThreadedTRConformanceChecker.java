...
public class MultiThreadedTRConformanceChecker implements Callable<MultiThreadedTRConformanceChecker> {
    /*static {
        try {
  ...
  private final Automaton logAutomaton;
  ...
  public double fitnessProblemsSolved = 0;

  public ExecutorService executor = null;

  //Constructor for callable
  public MultiThreadedTRConformanceChecker(int component, Automaton logAutomatonWithTRReduction,
      Automaton modelAutomaton, int numThreads) {
    this.component = component;
    this.logAutomaton = logAutomatonWithTRReduction;
    this.modelAutomaton = modelAutomaton;
    this.numThreads = numThreads;
  }

  //Conformance Checker for reduced Tandem Repeat traces
  public MultiThreadedTRConformanceChecker(String path, String log, String model, int numThreads) throws Exception {
    ...
    this.numThreads = numThreads;
    calculateOneOptimalAlignmentsWithTRreductionAndTraceEquivalence(Integer.MAX_VALUE);
    //calculateOneOptimalAlignmentsWithTandemRepeats(stateLimit);
    this.timeOneOptimal = System.currentTimeMillis() - start;
  }

  public MultiThreadedTRConformanceChecker(Automaton logAutomatonWithTrReductions, Automaton modelAutomaton,
      int stateLimit, int numThreads) {
    long start = System.currentTimeMillis();
  ...
  public MultiThreadedTRConformanceChecker call() {
    long start = System.nanoTime();
    this.psp = new PSP(this.logAutomaton, this.modelAutomaton);
    calculateOneOptimalAlignmentsWithTRreductionAndTraceEquivalence(Integer.MAX_VALUE);
...
//		this.timeOneOptimal = time.intValue();
  ...
  private void calculateOneOptimalAlignmentsWithTRreductionAndTraceEquivalence(int stateLimit) {
    ...
    //System.out.println("Min model cost : " + modelAutomaton.minNumberOfModelMoves());

    if (this.numThreads > 1) {
      executor = Executors.newFixedThreadPool(this.numThreads);
      //ExecutorService executor = Executors.newCachedThreadPool();
      FastList<Future<CheckTRConformanceCallable>> futures = new FastList<>();
      for (IntIntHashMap finalConfiguration : logAutomaton.getConfigDecoderMapping().keySet()) {
        for (IntArrayList trace : logAutomaton.getConfigDecoderMapping().get(finalConfiguration)) {
          CheckTRConformanceCallable confTask = new CheckTRConformanceCallable(psp, logAutomaton, modelAutomaton,
              finalConfiguration, trace, stateLimit, useVisited);
      ...
      for (Future<CheckTRConformanceCallable> fut : futures) {
        CheckTRConformanceCallable confResult = null;
        try {
          confResult = fut.get();
        } catch (Exception e) {
        ...
        this.resOneOptimal().addAll(confResult.results.values());
        this.useVisited = confResult.useVisited;
        fitnessProblemsSolved += confResult.fitnessProblemsSolved;
        for (DecodeTandemRepeats decoder : confResult.results.keySet()) {
          IntArrayList traces = new IntArrayList();
          traces.addAll(logAutomaton.caseTracesMapping.get(decoder.trace()).toArray());
          this.caseReplayResultMapping().put(traces, confResult.results.get(decoder));
          this.traceAlignmentsMapping.put(decoder.trace(), confResult.results.get(decoder));
        }
      }
    } else {
      for (IntIntHashMap finalConfiguration : logAutomaton.getConfigDecoderMapping().keySet()) {
  ...
  private void calculatePartiallySynchronizedPathsWithLeastSkipsFor(IntIntHashMap finalConfiguration,
    ...
    while (!toBeVisited.isEmpty()) {
      ...
      if (qStates != stateLimit) {
        if ((sizeMoveOnLog = currentNode.configuration().moveOnLog().size()) != traceLabels.size()) {
      ...
      if (currentNode.stLog().isFinal() && currentNode.stModel().isFinal() && currentNode.configuration().moveOnLog()
          .equals(traceLabels)) {
  ...
  private void calculatePartiallySynchronizedPathWithLeastSkipsFor(IntIntHashMap finalConfiguration,
    ...
    while (true) {
      ...
      if (qStates != stateLimit) {
        if ((sizeMoveOnLog = currentNode.configuration().moveOnLog().size()) != decoder.reducedTrace()
            .size())// && visited.size()<=stateLimit)
        {
          int expTraceLabel = decoder.reducedTrace().get(sizeMoveOnLog);
          for (Transition tlog : currentNode.stLog().outgoingTransitions()) {
            if (tlog.eventID() == expTraceLabel) {
              //see if match is possible
              for (Transition tmodel : currentNode.stModel().outgoingTransitions()) {
  ...
  private ReducedResult calculateReducedAlignmentFor(IntIntHashMap finalConfiguration, DecodeTandemRepeats decoder,
    ...
    double end = System.currentTimeMillis();
    useVisited = true;
    if (potentialFinalNodes.getFirst().weight() == Integer.MAX_VALUE) {
        ...
        if (tracePos == decoder.reductionStartPositions().get(posToReduce) - 1) {
          ...
          while (posTandemRepeat < lengthTandemRepeat) {
            ...
            if (tr.operation() == Configuration.Operation.RHIDE) {
              ...
              if (changesAllowed && (posToExtend == 0 || (posToExtend == lengthTandemRepeat - 1 && matchADDED))
                  && curStep == StepTypes.LMGOOD) {
  ...
  private void insertPartiallySynchronizedPathIntoPSP(IntIntHashMap finalConfiguration, DecodeTandemRepeats decoder,
      Node finalNode) {
      ...
      if (transition.operation() == Configuration.Operation.MATCH) {
        for (Transition tlog : currentNode.stLog().outgoingTransitions()) {
          for (Transition tmodel : currentNode.stModel().outgoingTransitions()) {
            if (tlog.eventID() == transition.eventLog() && tmodel.eventID() == transition.eventModel()
              ...
              potentialNode = initializePotentialNode(currentNode, tlog, tmodel, Configuration.Operation.MATCH,
                  finalConfiguration, true, decoder);
              break;
            }
          }
        ...
        for (Transition tlog : currentNode.stLog().outgoingTransitions()) {
          if (tlog.eventID() == transition.eventLog() && tlog.target().id() == transition.targetLog()) {
            potentialNode = initializePotentialNode(currentNode, tlog, null, Configuration.Operation.LHIDE,
                finalConfiguration, true, decoder); //, pw);
            break;
          }
        }
      } else {
    ...
    if (operation == Configuration.Operation.MATCH) {
      ...
      if (insertToPSP) {
        potentialNode = new Node(tlog.target(), currentNode.stModel(), potentialConfiguration,
            currentNode.tracePosition + 1, decoder.adjustedCost().get(currentNode.tracePosition));
