...
public class MTDecomposingTRConformanceChecker {
  ...
  private final int numTraceThreads;
  ...
  public MTDecomposingTRConformanceChecker(DecomposingTRImporter decompositions, int numThreads) {
  ...
  public MTDecomposingTRConformanceChecker(DecomposingTRImporter decompositions, int numTraceThreads,
      int numScompThreads) {
  ...
  private void checkConformance() {
    if (decompositions.doDecomposition) {
      ...
      componentAlignments.put(0,
          new MultiThreadedTRConformanceChecker(decompositions.dafsa, decompositions.modelFSM, Integer.MAX_VALUE,
              numTraceThreads));
  ...
  public void checkConformanceForComponents() {
    ...
    if (this.numScompThreads == 1) {
      ...
      compList.parallelStream().forEach(sComp -> componentAlignments.put(sComp,
          new MultiThreadedTRConformanceChecker(decompositions.componentDAFSAs.get(sComp),
              decompositions.sComponentFSMs.get(sComp), Integer.MAX_VALUE, numTraceThreads)));
    } else {
      ArrayList<Integer> compList = new ArrayList<Integer>();
      for (int pos = 0; pos < decompositions.sComponentFSMs.size(); pos++) {
        compList.add(pos);
      ...
      for (int pos = 0; pos < decompositions.sComponentFSMs.size(); pos++) {
        MultiThreadedTRConformanceChecker compTask = new MultiThreadedTRConformanceChecker(pos,
  ...
  public void recomposeConformance() {
    ...
    if (conflict > 0) {
      ...
      PNMatchInstancesRepResult rescueRes = new MultiThreadedTRConformanceChecker(
          new ImportEventLog().createReducedDAFSAfromLog(rescueLog, decompositions.globalInverseLabels),
          decompositions.modelFSM, Integer.MAX_VALUE, numTraceThreads).resOneOptimal();
