...
public class MultiThreadedDecomposedConformanceChecker {

  public DecomposingConformanceImporter decompositions;

  public UnifiedMap<Integer, MultiThreadedConformanceChecker> componentAlignments = new UnifiedMap<>();
  ...
  private int numTraceThreads = 1;

  public long start = System.nanoTime();

  public long TwithoutConflict;

  UnifiedMap<Integer, AllSyncReplayResult> projectedAlignments = new UnifiedMap<Integer, AllSyncReplayResult>();

  public MultiThreadedDecomposedConformanceChecker(DecomposingConformanceImporter decompositions, int numThreads)
      throws IOException {
    this.decompositions = decompositions;
    this.numTraceThreads = numThreads;
    //long start = System.nanoTime();
    if (decompositions.doDecomposition) {
      this.checkConformanceForComponents(numThreads, numThreads);
      this.recomposeConformance();
    } else {
      //System.out.println(decompositions.globalInverseLabels);
      componentAlignments.put(0, new MultiThreadedConformanceChecker(decompositions.componentDAFSAs.get(0),
          decompositions.sComponentFSMs.get(0), Integer.MAX_VALUE, numThreads));
      alignmentResult = componentAlignments.get(0).resOneOptimal;
      logSize = decompositions.xLog.size();
    }
  }

  public MultiThreadedDecomposedConformanceChecker(DecomposingConformanceImporter decompositions, int numSCompThreads,
      int numTraceThreads) throws IOException {
    this.decompositions = decompositions;
    this.numTraceThreads = numTraceThreads;
    //long start = System.nanoTime();
    if (decompositions.doDecomposition) {
      this.checkConformanceForComponents(numSCompThreads, numTraceThreads);
      this.recomposeConformance();
    } else {
      //System.out.println(decompositions.globalInverseLabels);
      componentAlignments.put(0,
          new MultiThreadedConformanceChecker(decompositions.dafsa, decompositions.modelFSM, Integer.MAX_VALUE,
              numTraceThreads));
      alignmentResult = componentAlignments.get(0).resOneOptimal;
      logSize = decompositions.xLog.size();
    }
  ...
  public void checkConformanceForComponents(int numSCompThreads, int numTraceThreads) {
    double start = System.currentTimeMillis();
    int component;
    //IntIntHashMap minModelLabels = new IntIntHashMap();
    this.minModelMoves = (int) Math.round(new SingleTraceConformanceChecker(new IntArrayList(), decompositions.modelFSM,
        decompositions.globalLabelMapping, decompositions.globalInverseLabels, new UnifiedMap<>()).res.getInfo()
        .get(PNRepResult.RAWFITNESSCOST));
    if (numSCompThreads > 1) {
      //parallelize here!
      ExecutorService executor = Executors.newFixedThreadPool(numSCompThreads);
      //ExecutorService executor = Executors.newCachedThreadPool();
      IntObjectHashMap<Future<MultiThreadedConformanceChecker>> futures = new IntObjectHashMap<>();
      for (int pos = 0; pos < decompositions.sComponentFSMs.size(); pos++) {
        MultiThreadedConformanceChecker compTask = new MultiThreadedConformanceChecker(pos,
            decompositions.componentDAFSAs.get(pos), decompositions.sComponentFSMs.get(pos), numTraceThreads);
        futures.put(pos, executor.submit(compTask));
      }
      executor.shutdown();
      ...
      for (int pos = 0; pos < decompositions.sComponentFSMs.size(); pos++) {
        MultiThreadedConformanceChecker compResult = null;
        try {
          compResult = futures.get(pos).get();
        } catch (Exception e) {
          System.exit(1);
        }
        componentAlignments.put(pos, compResult);
      }
    } else {
      for (int pos = 0; pos < decompositions.sComponentFSMs.size(); pos++) {
        MultiThreadedConformanceChecker compTask = new MultiThreadedConformanceChecker(pos,
            decompositions.componentDAFSAs.get(pos), decompositions.sComponentFSMs.get(pos), numTraceThreads);
        compTask.call();
        componentAlignments.put(pos, compTask);
      }
    }
  ...
  public void recomposeConformance() throws IOException {
    ...
    if (conflict > 0) {
      TwithoutConflict = TimeUnit.MILLISECONDS.convert(System.nanoTime() - this.start, TimeUnit.NANOSECONDS);
      //System.out.println(conflict);
      //PNRepResult rescueRes = AlignmentTest.computeCost((PetrinetGraph) decompositions.pnet, rescueLog, numTraceThreads);
      PNMatchInstancesRepResult rescueRes = new MultiThreadedConformanceChecker(
          new ImportEventLog().createDAFSAfromLog(rescueLog, decompositions.globalInverseLabels),
          this.decompositions.modelFSM, Integer.MAX_VALUE, numTraceThreads).resOneOptimal;
