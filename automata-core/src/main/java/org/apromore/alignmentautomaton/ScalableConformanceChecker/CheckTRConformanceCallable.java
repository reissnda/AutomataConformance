...
public class CheckTRConformanceCallable implements Callable<CheckTRConformanceCallable> {
  ...
  public CheckTRConformanceCallable(PSP psp, Automaton logAutomaton, Automaton modelAutomaton,
      IntIntHashMap finalConfiguration, IntArrayList reducedTrace, int stateLimit, boolean useVisited) {
    this.psp = psp;
  ...
  private void insertPartiallySynchronizedPathIntoPSP(IntIntHashMap finalConfiguration, DecodeTandemRepeats decoder,
    ...
    for (org.apromore.alignmentautomaton.psp.Synchronization transition : finalNode.configuration()
        .sequenceSynchronizations()) {
      ...
      DecodeTandemRepeats decoder) //, PrintWriter pw)
  {
    ...
    if (operation == Configuration.Operation.MATCH) {
      ...
      potentialConfiguration.sequenceSynchronizations().add(
          new org.apromore.alignmentautomaton.psp.Synchronization(Configuration.Operation.MATCH, tlog.eventID(),
              tmodel.eventID(), tlog.target().id(), tmodel.target().id(), currentNode.hashCode()));
      ...
      potentialConfiguration.sequenceSynchronizations().add(
          new org.apromore.alignmentautomaton.psp.Synchronization(Configuration.Operation.LHIDE, tlog.eventID(), -1,
              tlog.target().id(), -1, currentNode.hashCode()));
      //potentialConfiguration.logIDs().add(tlog.target().id());
      if (insertToPSP) {
      ...
      potentialConfiguration.sequenceSynchronizations().add(
          new org.apromore.alignmentautomaton.psp.Synchronization(Configuration.Operation.RHIDE, -1, tmodel.eventID(),
              -1, tmodel.target().id(), currentNode.hashCode()));
      //potentialConfiguration.modelIDs().add(tmodel.target().id());
      if (insertToPSP) {
