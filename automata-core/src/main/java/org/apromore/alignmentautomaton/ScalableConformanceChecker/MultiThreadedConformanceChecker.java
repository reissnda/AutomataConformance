...
public class MultiThreadedConformanceChecker implements Callable<MultiThreadedConformanceChecker> {
  ...
  private final Map<IntArrayList, AllSyncReplayResult> caseReplayResultMapping = new UnifiedMap<>();
  ...
  public MultiThreadedConformanceChecker(String path, String model, String log, int numThreads) throws Exception {
    ...
    Automaton modelAutomaton = new ImportProcessModel()
    ...
    Node source = new Node(logAutomaton.source(), modelAutomaton.source(),
        new Configuration(new IntArrayList(), new IntIntHashMap(), new IntArrayList(), new IntIntHashMap(),
            new FastList<org.apromore.alignmentautomaton.psp.Synchronization>(), new IntArrayList(),
            new IntArrayList()), 0);
  ...
  public MultiThreadedConformanceChecker(Automaton logAutomaton, Automaton modelAutomaton, int stateLimit,
      int numThreads) {
    this.nThreads = numThreads;
    long start = System.currentTimeMillis();
    this.logAutomaton = logAutomaton;
  ...
  private void calculateOneOptimalAlignments(int stateLimit) {
    ...
    if (this.nThreads > 1) {
      ...
      modelAutomaton.setMinNumberOfModelMoves(
          new SingleTraceConformanceChecker(new IntArrayList(), modelAutomaton, logAutomaton.eventLabels(),
              logAutomaton.inverseEventLabels(), logAutomaton.caseIDs).res.getInfo()
              .get(PNMatchInstancesRepResult.RAWFITNESSCOST).intValue());
      ...
      for (IntIntHashMap finalConfiguration : logAutomaton.configCasesMapping().keySet()) {
        for (IntArrayList trace : logAutomaton.configCasesMapping().get(finalConfiguration)) {
          CheckConformanceCallable confTask = new CheckConformanceCallable(psp, logAutomaton, modelAutomaton, trace,
              finalConfiguration, stateLimit, useVisited);
