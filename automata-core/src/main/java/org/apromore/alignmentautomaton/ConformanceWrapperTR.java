...
import org.apromore.alignmentautomaton.ScalableConformanceChecker.DecomposingTRConformanceChecker;
...
import org.apromore.alignmentautomaton.ScalableConformanceChecker.TRConformanceChecker;
...
import org.apromore.alignmentautomaton.importer.TRImporter;
...
public class ConformanceWrapperTR implements Callable<ConformanceWrapperTR> {
  ...
  public ConformanceWrapperTR call() throws Exception {
    if (type.equals("S-Components")) {
      ...
      PNRepResult res = emeq_alignments
          .doReplay(xLog, pnet, initMarking, finalMarking, summary.getEventClasses(), mapping, 1);
      ...
      PNRepResult res = alignments.computeCost(pnet, xLog, 1);
      time = TimeUnit.MILLISECONDS.convert(System.nanoTime() - start, TimeUnit.NANOSECONDS);
      ...
      fitnessProblemsSolved = Double
          .parseDouble(alignresult.getInfo().get(PNMatchInstancesRepResult.NUMALIGNMENTS));
  ...
  private void runHybrid() throws Exception {
