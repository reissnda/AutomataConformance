...
import org.apromore.alignmentautomaton.ScalableConformanceChecker.MTDecomposingTRConformanceChecker;
...
import org.apromore.alignmentautomaton.ScalableConformanceChecker.MultiThreadedTRConformanceChecker;
  ...
  public ConformanceWrapperTRMT call() throws Exception {
    try {
      if (type.equals("S-Components")) {
        ...
        pro = new MultiThreadedTRConformanceChecker(importer.logAutomaton, importer.modelAutomaton, Integer.MAX_VALUE,
            numTraceThreads);
        time = TimeUnit.MILLISECONDS.convert(System.nanoTime() - start, TimeUnit.NANOSECONDS);
        ...
        fitnessProblemsSolved = Double
            .parseDouble(pnresult.getInfo().get(PNMatchInstancesRepResult.NUMALIGNMENTS));
