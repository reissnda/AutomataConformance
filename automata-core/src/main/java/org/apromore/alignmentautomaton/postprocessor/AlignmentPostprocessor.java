...
public class AlignmentPostprocessor {
  ...
  public static Map<IntArrayList, AllSyncReplayResult> computeEnhancedAlignments(
      Map<IntArrayList, AllSyncReplayResult> alignments, Automaton originalAutomaton) {
  ...
  private static AllSyncReplayResult getEnhancedAlignment(AllSyncReplayResult alignment, Automaton automaton) {
    ...
    for (int i = 0; i < alignment.getStepTypesLst().get(0).size(); i++) {
      ...
      if (stepType == StepTypes.LMGOOD || stepType == StepTypes.MREAL) {
        ...
        var availableTransitions = info.keySet().stream()
            .filter(transition -> transition != null && !avoid.contains(transition)).collect(Collectors.toList());
        ...
        Transition nextTransition;
          ...
          if (idx != -1) {
              ...
              while (idx == -1) {
                  ...
                  availableTransitions = info.keySet().stream().filter(transition -> transition != null && !avoid.contains(transition))
    ...
    if (currentState != null && !currentState.isFinal()) {
      var gateways = gatewaysInfo.get(currentState).get(null).stream().map(Transition::eventID)
          .collect(Collectors.toList());
    ...
    AllSyncReplayResult enhancedAlignment = new AllSyncReplayResult(nodeInstanceLsts, stepTypesLsts, 0,
        alignment.isReliable());
  ...
  private static LinkedHashMap<State, LinkedHashMap<Transition, List<Transition>>> computeGatewaysInfo(
      Automaton automaton) {
