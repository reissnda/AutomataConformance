...
//  }
  ...
  public static void test(String[] args) throws Exception {
    ...
    for (ASynchronousMoveSorting sort : new ASynchronousMoveSorting[] { ASynchronousMoveSorting.TOTAL,
      ...
      AlignmentTest
          .computeCost(costMOS, costMOT, initialMarking, finalMarkings, new TestPluginContext(), net, log, mapping,
              true, sort);
      long mid = System.currentTimeMillis();
      System.out.println("   With ILP cost: " + "  t: " + (mid - start));
  ...
  public static PNRepResult computeCost(PetrinetGraph net, XLog log) {
    ...
    return AlignmentTest
        .computeCost(costMOS, costMOT, initialMarking, finalMarkings, new TestPluginContext(), net, log, mapping, true,
            ASynchronousMoveSorting.LOGMOVEFIRST);
  }
  ...
  public static PNRepResult computeCost(Map<Transition, Integer> costMOS, Map<XEventClass, Integer> costMOT,
      ...
      TransEvClassMapping mapping, boolean useILP, ASynchronousMoveSorting sorting, int numThreads) {
    ...
    try {
      ...
      for (SyncReplayResult res : result) {
        if (res.isReliable()) {
          ...
          cost += ((int) res.getInfo().get(PNRepResult.RAWFITNESSCOST).doubleValue()) * res.getTraceIndex().size();
        } else {
