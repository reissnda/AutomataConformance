...
public class XFactorySingletonImpl implements XFactory {
  ...
  private final XID fakeXID = XIDFactory.instance().createId();

  private final UnifiedMap<String, String> map = new UnifiedMap<>();

  private final LongObjectHashMap<Date> dateMap = new LongObjectHashMap<>();

  private final DoubleObjectHashMap<Double> doubleMap = new DoubleObjectHashMap<>();

  private final LongObjectHashMap<Long> longMap = new LongObjectHashMap<>();
  ...
  public XLog createLog() {
    if (useLinkedList) {
      return new XLogLinkedListSingletonImpl(new XAttributeMapSingletonImpl());
    } else {
      return new XLogArrayListSingletonImpl(new XAttributeMapSingletonImpl());
    }
  }

  public XLog createLog(XAttributeMap attributes) {
    if (useLinkedList) {
      return new XLogLinkedListSingletonImpl(attributes);
    } else {
      return new XLogArrayListSingletonImpl(attributes);
    }
  }

  public XTrace createTrace() {
    if (useLinkedList) {
      return new XTraceLinkedListSingletonImpl(new XAttributeMapSingletonImpl());
    } else {
      return new XTraceArrayListSingletonImpl(new XAttributeMapSingletonImpl());
    }
  }

  public XTrace createTrace(XAttributeMap attributes) {
    if (useLinkedList) {
      return new XTraceLinkedListSingletonImpl(attributes);
    } else {
      return new XTraceArrayListSingletonImpl(attributes);
    }
  }
