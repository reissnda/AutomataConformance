...
public final class Prim {
   ...
   * @param result
   *     The resulting graph, used to check what has been visited
   *     so far.
   */
  private static <T> T minCostEndpoint(T node, UndirectedGraphKruskal<T> graph, UndirectedGraphKruskal<T> result) {
   ...
   * @param graph
   *     The graph whose MST is being computed, used so we can
   *     get the edges to consider.
   * @param pq
   *     The Fibonacci heap holding each endpoint.
   * @param result
   *     The result graph.  We need this information so that we
   ...
   *     elements.
   */
  private static <T> void addOutgoingEdges(T node, UndirectedGraphKruskal<T> graph, FibonacciHeap<T> pq,
      UndirectedGraphKruskal<T> result, Map<T, FibonacciHeap.Entry<T>> entries) {
