...
public final class EdmondsMatching {
   ...
   *
   * @param path
   *     The alternating path linking the exposed endpoints.
   * @param m
   *     The matching to update, which is an in/out parameter.
   */
  ...
  private static final class NodeInformation<T> {
     ...
     * @param isOuter
     *     Whether the given node is an outer node.
     */
  ...
  private static final class Blossom<T> {
     ...
     *
     * @param root
     *     The root node of the blossom.
     * @param cycle
     *     The nodes of the cycle listed in the order in which
     *     they appear in the blossom.
     * @param nodes
     *     The nodes in the cycle, stored as a set.
     */
   ...
   *
   * @param g
   *     The graph in which to search for the path.
   * @param m
   *     A matching in that graph.
   * @return An alternating path in g, or null if none exists.
   */
  private static <T> List<T> findAlternatingPath(UndirectedGraphEdmonds<T> g, UndirectedGraphEdmonds<T> m) {
    ...
    while (!worklist.isEmpty()) {
      ...
      if (endInfo != null) {
        ...
        if (endInfo.isOuter && startInfo.treeRoot == endInfo.treeRoot) {
           ...
           */
          List<T> path = findAlternatingPath(contractGraph(g, blossom), contractGraph(m, blossom));
       ...
       */
         ...
         * root as its parent.
         */
        forest.put(curr.end, new NodeInformation<T>(curr.start, startInfo.treeRoot, false));
        ...
        forest.put(endpoint, new NodeInformation<T>(curr.end, startInfo.treeRoot, true));
   ...
   * @param edge
   *     The edge that created a cycle.
   * @return A Blossom struct holding information about then blossom.
   */
  private static <T> Blossom<T> findBlossom(Map<T, NodeInformation<T>> forest, Edge<T> edge) {
   ...
   * @param blossom
   *     The set of nodes in the blossom.
   * @return The contraction g / blossom.
   */
  private static <T> UndirectedGraphEdmonds<T> contractGraph(UndirectedGraphEdmonds<T> g, Blossom<T> blossom) {
   ...
   *
   * @param path
   *     The path in the contracted graph.
   * @param g
   *     The uncontracted graph.
   * @param forest
   *     The alternating forest of the original graph.
   * @param blossom
   *     The blossom that was contracted.
   * @return An alternating path in the original graph.
   */
  private static <T> List<T> expandPath(List<T> path, UndirectedGraphEdmonds<T> g, Map<T, NodeInformation<T>> forest,
      Blossom<T> blossom) {
   ...
   *
   * @param g
   *     The graph in which the cycle occurs.
   * @param blossom
   *     The blossom in that graph.
   * @param node
   *     The node outside of the blossom.
   * @return Some node in the blossom with an edge in g to the indicated
   *     node.
   */
  private static <T> T findNodeLeavingCycle(UndirectedGraphEdmonds<T> g, Blossom<T> blossom, T node) {
