...
package com.raffaeleconforti.util;
...
public class LogOptimizer {
  ...
  private XAttribute createXAttribute(String key, Object value, XAttribute originalAttribute) {
    XAttribute result;
    if (originalAttribute instanceof XAttributeLiteral) {
      result = factory.createAttributeLiteral(key, (String) value, null);
    } else if (originalAttribute instanceof XAttributeBoolean) {
      result = factory.createAttributeBoolean(key, (Boolean) value, null);
    } else if (originalAttribute instanceof XAttributeDiscrete) {
      result = factory.createAttributeDiscrete(key, (Long) value, null);
    } else if (originalAttribute instanceof XAttributeContinuous) {
      result = factory.createAttributeContinuous(key, (Double) value, null);
    } else if (originalAttribute instanceof XAttributeTimestamp) {
      result = factory.createAttributeTimestamp(key, (Date) value, null);
    } else if (originalAttribute instanceof XAttributeID) {
      result = factory.createAttributeID(key, (XID) value, null);
    } else {
