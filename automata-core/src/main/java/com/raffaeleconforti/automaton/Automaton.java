...
public class Automaton<T> implements Cloneable {
  ...
  public void addEdge(Node<T> source, Node<T> target) {
    ...
    edges.put(edge, val);
    if (directedGraph != null) {
      directedGraph.addEdge(source, target, 1);
    }
  ...
  public void removeEdge(Edge<T> edge) {
    ...
    if (val <= 0.0) {
      ...
      edges.remove(edge);
      if (directedGraph != null) {
        directedGraph.removeEdge(edge.getSource(), edge.getTarget());
      }
      return;
    }
    edges.put(edge, val);
  ...
  public Object clone() {
    ...
    if (start != null) {
      clone.start = new UnifiedSet<Node<T>>(start);
    }
    if (end != null) {
      clone.end = new UnifiedSet<Node<T>>(end);
    }
    if (directedGraph != null) {
      clone.directedGraph = directedGraph.clone();
    }
    return clone;
  }
