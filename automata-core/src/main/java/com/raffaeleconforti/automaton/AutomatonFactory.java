...
import com.raffaeleconforti.util.NameExtractor;
...
public class AutomatonFactory {
  ...
  public Automaton generateForTimeFilter(XLog log, Map<String, Set<String>> duplicatedEvents) {
    ...
    for (XTrace t : log) {
      for (XEvent e : t) {
        if (duplicatedEvents.get(nameExtractor.getTraceName(t)) == null || !duplicatedEvents
            .get(nameExtractor.getTraceName(t)).contains(nameExtractor.getEventName(e))) {
          Node n;
          if ((n = map.get(new Node(nameExtractor.getEventName(e)))) == null) {
            n = new Node(nameExtractor.getEventName(e));
            map.put(n, n);
      ...
      for (int i = 0; i < t.size(); i++) {
        ...
        if (duplicatedEvents.get(nameExtractor.getTraceName(t)) == null || !duplicatedEvents
          ...
          if (i < t.size() - 1) {
            XEvent e1 = t.get(i + 1);
            if (duplicatedEvents.get(nameExtractor.getTraceName(t)) == null || !duplicatedEvents
                .get(nameExtractor.getTraceName(t)).contains(nameExtractor.getEventName(e1))) {
              Node n = map.get(new Node(nameExtractor.getEventName(e)));
              Node n1 = map.get(new Node(nameExtractor.getEventName(e1)));
