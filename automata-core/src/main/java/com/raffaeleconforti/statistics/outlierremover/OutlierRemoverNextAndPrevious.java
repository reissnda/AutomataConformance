...
public class OutlierRemoverNextAndPrevious extends OutlierRemoverAbstract {
  ...
  public OutlierRemoverNextAndPrevious(OutlierIdentifierGenerator outlierIdentifierGenerator,
  ...
  public void selectOulierToRemove(XLog log, int lookAHead, boolean selectOnlyOneOutlier, boolean smallestOrLargest) {
    if (selectOnlyOneOutlier) {
      boolean outlearsFound = mapOutliers.size() > 0;
  ...
  @Override
  public void selectOulierToRemoveReverse(XLog log, int lookAHead, boolean selectOnlyOneOutlier,
      boolean smallestOrLargest) {
    if (selectOnlyOneOutlier) {
      boolean outlearsFound = mapOutliers.size() > 0;
  ...
  private void checkIfOutlier(Map<Outlier<String>, Integer> removed, XTrace t, int i, int lookAHead,
      boolean directOrReverse) {
  ...
  @Override
  public XLog generateNewLog(XLog log, OutlierIdentifierGenerator<String> outlierIdentifierGenerator, int lookAHead,
      boolean selectOnlyOneOutlier) {
    boolean outlearsFound = mapOutliers.size() > 0;
  ...
  @Override
  public XLog generateNewLogReverse(XLog log, OutlierIdentifierGenerator<String> outlierIdentifierGenerator,
      int lookAHead, boolean selectOnlyOneOutlier) {
    boolean outlearsFound = mapOutliers.size() > 0;
  ...
  private void removeOutlierSelectOnlyOne(XTrace t, XTrace newT, int i, int lookAHead, boolean directOrReverse) {
    ...
    if (!nameCurrent.equals(((SingleOutlierIdentifier) outlier.getIdentifier()).getIdentifier()) || !nameNext
        .equals(outlier.getElementToRemove())) {
