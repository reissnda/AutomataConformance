...
public class TraceReplayTask implements Callable<TraceReplayTask> {
  ...
  private final int maximumNumberOfStates;

  private final int[] eventsWithErrors;
  ...
  public TraceReplayTask(Replayer replayer, ReplayerParameters parameters, XTrace trace, int traceIndex,
      int timeoutMilliseconds, int maximumNumberOfStates, long preProcessTimeNanoseconds, int... eventsWithErrors) {
    this.replayer = replayer;
    this.parameters = parameters;
    this.trace = trace;
    ...
    if (original < 0) {
      ...
      if (product != null) {
        if (parameters.debug == Debug.DOT) {
          Utils.toDot(product, Debug.getOutputStream());
        }
        ...
        alignment = algorithm
            .run(this.replayer.getProgress(), timeoutMilliseconds, maximumNumberOfStates, parameters.costUpperBound);

        if (parameters.debug == Debug.DOT) {
          Utils.toDot(product, alignment, Debug.getOutputStream());
        }
  ...
  private SyncReplayResult toSyncReplayResult(SyncProduct product, TObjectIntMap<Statistic> statistics, int[] alignment,
      XTrace trace, int traceIndex, List<Transition> transitionList) {
    ...
    for (int i = 0; i < alignment.length; i++) {
      ...
      if (product.getTypeOf(t) == SyncProduct.LOG_MOVE) {
        nodeInstance.add(((XAttributeLiteral) trace.get(product.getEventOf(t)).getAttributes().get("concept:name"))
            .getValue());
        stepTypes.add(StepTypes.L);
        lm += product.getCost(t);
      } else {
        nodeInstance.add(transitionList.get(t).getLabel());
    ...
    switch (parameters.algorithm) {
      ...
      case INCREMENTALASTAR:
        if (parameters.initialSplits > 0 && eventsWithErrors.length == 0) {
          ...
          return new AStarLargeLP(product, parameters.moveSort, parameters.useInt, parameters.debug, eventsWithErrors);
        }
      case DIJKSTRA:
