...
public interface ReplayAlgorithm {
   ...
   * after a call to run();
   */
  TObjectIntMap<Statistic> getStatistics();
   ...
   *     computations.
   */
  int[] run(Canceler canceler, int timeoutMilliseconds, int maximumNumberOfStates, int costUpperLimit)
      throws LPMatrixException;
   ...
   */
  void putStatistic(Statistic stat, int value);
   ...
   */
  boolean equalMarking(int markingId, byte[] marking);
   ...
   */
  boolean hasExactHeuristic(int markingId);
   ...
   */
  int getLastRankOf(int markingId);
   ...
   */
  boolean isClosed(int markingId);
