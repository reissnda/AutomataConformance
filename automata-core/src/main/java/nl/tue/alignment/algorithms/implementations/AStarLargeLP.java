...
public class AStarLargeLP extends AbstractLPBasedAlgorithm {
  ...
  private static final Random random = new Random(41343);
  ...
  private final int modelMoves;

  private final SyncProduct product;
  ...
  private void init() throws LPMatrixException {
    ...
    try {
      ...
      for (; r <= rows; r++) {
        ...
        solver.setRh(r,
            product.getFinalMarking()[(r - 2) % product.numPlaces()] - product.getInitialMarking()[(r - 2) % product
                .numPlaces()]);
        coefficients++;
      }
      ...
      while (it.hasNext()) {
        ...
        for (int i = 0; i < output.length; i++) {
          if (full) {
            ...
            for (int p = 1 + start + product.numPlaces() + output[i]; p < col.length; p += product.numPlaces()) {
              col[p] += 1;
              coefficients++;
            }
          }
  ...
  private int getExactHeuristic(LpSolve solver, int marking, byte[] markingArray, int markingBlock, int markingIndex,
    ...
    try {
      ...
      for (; r <= rows; r++) {
        solver.setRh(r,
            product.getFinalMarking()[(r - 2) % product.numPlaces()] - markingArray[(r - 2) % product.numPlaces()]);
      }
