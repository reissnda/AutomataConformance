...
public class ConstraintSet {
  ...
  private final TIntObjectMap<Set<Constraint>> label2input = new TIntObjectHashMap<>();

  private final TIntObjectMap<Set<Constraint>> label2output = new TIntObjectHashMap<>();
  ...
  public ConstraintSet(Petrinet net, Marking initialMarking, XEventClasses classes, TObjectIntMap<XEventClass> c2id,
    ...
    for (int r = cs + ts; r < rows; r++) {
      for (int r2 = r + 1; r2 < rows; r2++) {
        if (firstNonZero[r2] < firstNonZero[r] || (firstNonZero[r2] == firstNonZero[r] && firstNonZero[r] < columns
            && matrix[r2][firstNonZero[r]] < matrix[r][firstNonZero[r]])) {
          // swap rows
          int[] row = matrix[r2];
          matrix[r2] = matrix[r];
          matrix[r] = row;
