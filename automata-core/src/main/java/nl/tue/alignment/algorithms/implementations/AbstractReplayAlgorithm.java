...
abstract class AbstractReplayAlgorithm extends AbstractReplayAlgorithmDataStore {
  ...
  private final boolean queueSorting;
  ...
  protected TObjectIntMap<Statistic> replayStatistics;

  public AbstractReplayAlgorithm(SyncProduct product, boolean moveSorting, boolean queueSorting, boolean preferExact) {
   ...
   */
  @Override
  public TObjectIntMap<Statistic> getStatistics() {
    return replayStatistics;
  }

  protected void fillStatistics(int[] alignment) {
    replayStatistics.put(Statistic.POLLACTIONS, pollActions);
    replayStatistics.put(Statistic.CLOSEDACTIONS, closedActions);
    replayStatistics.put(Statistic.QUEUEACTIONS, queueActions);
    replayStatistics.put(Statistic.EDGESTRAVERSED, edgesTraversed);
    replayStatistics.put(Statistic.MARKINGSREACHED, markingsReached);
    replayStatistics.put(Statistic.HEURISTICSCOMPUTED, heuristicsComputed);
    replayStatistics.put(Statistic.HEURISTICSDERIVED, heuristicsDerived);
    replayStatistics.put(Statistic.HEURISTICSESTIMATED, heuristicsEstimated);
    replayStatistics.put(Statistic.ALIGNMENTLENGTH, alignmentLength);
    replayStatistics.put(Statistic.COST, alignmentCost);
    replayStatistics.put(Statistic.EXITCODE, alignmentResult);
    replayStatistics.put(Statistic.RUNTIME, runTime);
    replayStatistics.put(Statistic.SETUPTIME, setupTime);
    replayStatistics.put(Statistic.TOTALTIME, setupTime + runTime);
    replayStatistics.put(Statistic.MAXQUEUELENGTH, queue.maxSize());
    replayStatistics.put(Statistic.MAXQUEUECAPACITY, queue.maxCapacity());
    replayStatistics.put(Statistic.VISITEDSETCAPACITY, visited.capacity());
    replayStatistics.put(Statistic.TRACELENGTH, net.numEvents());
    replayStatistics.put(Statistic.PLACES, net.numPlaces());
    replayStatistics.put(Statistic.TRANSITIONS, net.numTransitions());
    replayStatistics.put(Statistic.LMCOST, getCostForType(alignment, SyncProduct.LOG_MOVE, SyncProduct.LOG_MOVE));
    replayStatistics.put(Statistic.MMCOST, getCostForType(alignment, SyncProduct.MODEL_MOVE, SyncProduct.TAU_MOVE));
    replayStatistics.put(Statistic.SMCOST, getCostForType(alignment, SyncProduct.SYNC_MOVE, SyncProduct.SYNC_MOVE));
    replayStatistics.put(Statistic.MEMORYUSED, (int) (getEstimatedMemorySize() / 1024));
  }
  ...
  protected int[] runReplayAlgorithm(Canceler canceller, long startTime, int timeoutMilliseconds, int costUpperBound)
    ...
    do {
      ...
      try {
        ...
        queueLoop:
        while (!queue.isEmpty() && (System.currentTimeMillis() < timeoutAtTimeInMillisecond)
  ...
  protected void writeStatus() {
    ...
    debug.println(Debug.NORMAL,
        "   FScore head:     " + getFScore(queue.peek()) + " = G: " + getGScore(queue.peek()) + " + H: " + getHScore(
            queue.peek()));
    ...
    debug.println(Debug.NORMAL,
        "   Heuristics est  :" + String.format("%,d", (markingsReached - heuristicsComputed - heuristicsDerived)));
