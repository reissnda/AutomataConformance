...
class IntegerUkkonenSuffixTree {
    ...
    private int SEA(SuffixTreePath streePath, boolean afterRule3) {
        ...
        if (pos.node == root) {
            ...
            if (isLastSymbolInEdge()) {
                tempNode = findChild(IntegerUkkonenSuffixTree.subList(sequence, streePath.end
                       , (streePath.end + 1)));
                if (tempNode != null) {
                    pos.node = tempNode;
                    pos.edgePosition = 0;
                ...
                IntArrayList str1 = IntegerUkkonenSuffixTree.subList(sequence, tempIndex,
                        tempIndex + 1);
                IntArrayList str2 = IntegerUkkonenSuffixTree.subList(sequence,
                        streePath.end, streePath.end + 1);
                if (str1.equals(str2)) {
                    pos.edgePosition++;
                    noSymbolsFound = 1;
                }
    ...
    private int traceString(SuffixTreePath streePath, IntArrayList searchString,
                            boolean skip) {
        /*
         * This variable will be true when search is done. It is a return value
         * from function traceSingleEdge
    ...
    private TraceSingleEdgeResult traceSingleEdge(SuffixTreePath streePath, boolean skip) {
        ...
        if (skip) {
            ...
            for (edgePos = 1, noEdgeSymbolsFound = 1; edgePos < nodeLabelLength; edgePos++, noEdgeSymbolsFound++) {
                 ...
                 * equal - continue
                 */
                if (!IntegerUkkonenSuffixTree.subList(sequence,
                        pos.node.edgeLabelStart + edgePos,
                        pos.node.edgeLabelStart + edgePos + 1).equals(
                        IntegerUkkonenSuffixTree.subList(sequence,streePath.begin + edgePos,
    ...
    private TraceSingleEdgeResult traceSingleEdge(SuffixTreePath streePath, IntArrayList searchString, boolean skip) {
        ...
        SuffixNode contNode = findChild(IntegerUkkonenSuffixTree.subList(searchString,streePath.begin
                , streePath.begin + 1));
        ...
        if (skip) {
            ...
            for (edgePos = 1, noEdgeSymbolsFound = 1; edgePos < nodeLabelLength; edgePos++, noEdgeSymbolsFound++) {
                ...
                if (!IntegerUkkonenSuffixTree.subList(sequence,
                        ...
                        IntegerUkkonenSuffixTree.subList(searchString,(streePath.begin + edgePos)
                                        ,
                                (streePath.begin + edgePos + 1)
                                        ))) {
                    edgePos--;
    ...
    private SuffixNode findChild(IntArrayList symbol) {
        ...
        for (i = 0; i < noChildren; i++) {
            IntArrayList lst = IntegerUkkonenSuffixTree.subList(sequence, pos.node.childrenList.get(i).edgeLabelStart, (pos.node.childrenList.get(i).edgeLabelStart + 1));
    ...
    private void printNode(SuffixNode node, int depth) {
        ...
        if (depth > 0) {
            ...
            while (start <= end) {
                System.out.print(IntegerUkkonenSuffixTree.subList(sequence,start,
                        (start + 1)));
                start++;
            }
    ...
    public void findLeftDiverseNodes() {
         ...
         */
        for (SuffixNode leafNode : leaves) {
            leafNode.leftSymbol = IntegerUkkonenSuffixTree.subList(sequence,
                    (leafNode.pathPosition - 1),
                    leafNode.pathPosition);
    ...
    public HashSet<IntArrayList> getMaximalRepeats() {
        ...
        for (SuffixNode leftDiverseNode : leftDiverseNodeList) {
            maximalRepeats.add(IntegerUkkonenSuffixTree.subList(sequence,leftDiverseNode.pathPosition
                    , (leftDiverseNode.edgeLabelEnd + 1)
                    ));
        }
    ...
    public HashSet<IntArrayList> getSuperMaximalRepeats() {
        ...
        for (SuffixNode leftDiverseNode : leftDiverseNodeList) {
            ...
            if (isAllChildrenLeaves
                    ...
                    .size()) {
                superMaximalRepeats.add(IntegerUkkonenSuffixTree.subList(sequence,
                        leftDiverseNode.pathPosition,
                        (leftDiverseNode.edgeLabelEnd + 1)));
            }
        }
    ...
    public HashSet<IntArrayList> getNearSuperMaximalRepeats() {
        ...
        for (SuffixNode leftDiverseNode : leftDiverseNodeList) {
            ...
            if (isNearSuperMaximal) {
                nearSuperMaximalRepeats.add(IntegerUkkonenSuffixTree.subList(sequence,
                        leftDiverseNode.pathPosition,
                        (leftDiverseNode.edgeLabelEnd + 1)));
            }
        }
    ...
    public void LZDecomposition() {
        ...
        String strS = "s: ";
        for(int i = 0; i < s.length; i++) strS += s[i] + " ";
        System.out.println(strS);
        String strL = "l: ";
        for(int i = 0; i < l.length; i++) strL += l[i] + " ";
        ...
        for (int i = 0; i < noBlocks; i++) {
            blocks.add(IntegerUkkonenSuffixTree.subList(sequence,I.get(i), I
                    .get(i + 1)
                    ));
    ...
    private void processBlockAlgorithm1A(int currentBlockLength, int h1) {
        int q, k1, k2;
        System.out.println("Pos: " + (h1-currentBlockLength) + ", Block length: " + currentBlockLength + ", nextBlockStart: " + h1);
        for (int k = 1; k <= currentBlockLength; k++) {
            ...
            while ((h1 + k1 + 1 < sequenceLength)
                    && IntegerUkkonenSuffixTree.subList(sequence,(q + k1),
                    (q + k1 + 1)).equals(
                    IntegerUkkonenSuffixTree.subList(sequence,(h1 + k1), (h1
                            + k1 + 1)
                            )))
                k1++;
            ...
            k2 = 0;
            while ((q - k2 - 1) > 0
                    && IntegerUkkonenSuffixTree.subList(sequence, (q - 1 - k2),
                    (q - k2)).equals(
                    IntegerUkkonenSuffixTree.subList(sequence,(h1 - k2 - 1),
                            (h1 - k2))))
                k2++;
    ...
    private void processBlockAlgorithm1B(int currentBlockLength,
        ...
        for (int k = 1; k <= currentBlockLength + nextBlockLength; k++) {
            ...
            while ((q + k1) < sequenceLength
                    && IntegerUkkonenSuffixTree.subList(sequence,(h + k1),
                    (h + k1 + 1)).equals(
                    IntegerUkkonenSuffixTree.subList(sequence,(q + k1), (q
                            + k1 + 1)
                            )))
                k1++;
            ...
            k2 = 0;
            while ((h - k2 - 1) > 0
                    && IntegerUkkonenSuffixTree.subList(sequence,(h - k2 - 1),
                    (h - k2)).equals(
                    IntegerUkkonenSuffixTree.subList(sequence,(q - 1 - k2),
                            (q - k2))))
                k2++;
        ...
        for (int i = 0; i < sequenceLength; i++) {
            if (tandemPairs[i].size() > 0) {
                for (Integer loc : tandemPairs[i]) {
                    tandemRepeat = IntegerUkkonenSuffixTree.subList(sequence,(i + 1) ,
                            (i + 1 + loc));
     ...
     */
     public HashMap<HashSet<IntArrayList>, HashSet<IntArrayList>> getPrimitiveTandemRepeats() {
        ...
        for (IntArrayList tandemRepeatPair : tandemRepeatSet.get(pos)) {
            ...
            for (int i = 0; i < tandemRepeatLength; i++) {
                tandemRepeatAlphabetSet.add(IntegerUkkonenSuffixTree.subList(tandemRepeat,i
                        , (i + 1)));
            }
...
//					System.out.println(tr);
                    ...
                    if (found) {
                        ...
                        complexTandemRepeats.add(IntegerUkkonenSuffixTree.subList(tr,
                                0, tandemRepeatLength / 2));
                    } else {
...
//				+ " msecs to find the maximal repeats in the string");
	...
	*/
    ...
    {
        if(end < begin || begin < 0 || end > ls.size()) return null;
