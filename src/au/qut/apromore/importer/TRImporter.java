...
public class TRImporter
{
    ...
    public void createAutomata() throws Exception
    {
        ...
        this.modelAutomaton = this.modelImporter.createFSMfromPetrinet(this.pnet, this.initMarking, this.logAutomaton.eventLabels(), this.logAutomaton.inverseEventLabels());
    }
    ...
    public void gatherTRStatistics()
    {
                ...
                for (DecodeTandemRepeats decoder : logAutomaton.reductions.get(reducedTr).get(reduction)) {
                    ...
                    for (int redPos = 0; redPos < decoder.reductionStartPositions().size(); redPos++) {
                        if (decoder.reductionOriginalLength().get(redPos) > decoder.reductionCollapsedLength().get(redPos)) {
                            ...
                            lengths.add(length);
                            nReduced.add(decoder.reductionOriginalLength().get(redPos)-decoder.reductionCollapsedLength().get(redPos));
        ...
        if(nTRrepeatTpls.size()==0) nTRrepeatTpls.add(0);
        if(nTRrepsList.size()==0) nTRrepsList.add(0);
        if(lengths.size()==0) lengths.add(0);
        if(nReduced.size()==0) nReduced.add(0);
        if(originalLengths.size()==0) originalLengths.add(0);
        if(collapsedLengths.size()==0) collapsedLengths.add(0);
        ...
        System.out.println(nTRrepeatTpls.average() + ", " + nTRrepsList.average() + ", " + lengths.average() + ", "  + nReduced.average() + ", " + originalLengths.average() + ", " + collapsedLengths.average());
    }
    ...
    private void recordModelStatistics() throws Exception
    {
        ...
        String result = path + "," + model + "," + stats.places + "," + stats.transitions + "," + stats.arcs + "," + stats.size + "," + stats.choice + "," + stats.parallel + ","
                + stats.rg_nodes + "," + stats.rg_arcs + "," + stats.rg_size;
        if(stats.doDecomposition)
            result += ", " + stats.scompRGSize.size() + "," + stats.scompRGSize.average() + "," + stats.scompRGNodes.average() + "," + stats.scompRGArcs.average() + "\n";
        else result += ",1\n";
        System.out.println(headlines);
        System.out.println(result);
        FileWriter pw = null;
        ...
        if(pw==null) pw = new FileWriter(path + "model_stats.txt", true);
        pw.append(result);
        pw.close();
    }
}
