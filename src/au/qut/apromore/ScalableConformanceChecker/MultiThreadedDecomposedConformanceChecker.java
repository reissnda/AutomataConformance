...
public class MultiThreadedDecomposedConformanceChecker
{
    ...
    public MultiThreadedDecomposedConformanceChecker(DecomposingConformanceImporter decompositions, int numThreads) throws Exception
    {
        ...
        else
        {
            ...
            componentAlignments.put(0,new MultiThreadedConformanceChecker(decompositions.componentDAFSAs.get(0), decompositions.sComponentFSMs.get(0),Integer.MAX_VALUE,numThreads));
            alignmentResult = componentAlignments.get(0).resOneOptimal;
            logSize = decompositions.xLog.size();
        }
    }

    public MultiThreadedDecomposedConformanceChecker(DecomposingConformanceImporter decompositions, int numSCompThreads, int numTraceThreads) throws Exception
    {
        this.decompositions = decompositions;
        ...
        {
            //System.out.println(decompositions.globalInverseLabels);
            componentAlignments.put(0,new MultiThreadedConformanceChecker(decompositions.dafsa, decompositions.modelFSM,Integer.MAX_VALUE,numTraceThreads));
            alignmentResult = componentAlignments.get(0).resOneOptimal;
            logSize = decompositions.xLog.size();
        }
    }
    ...
    public void checkConformanceForComponents(int numSCompThreads, int numTraceThreads)
    {
        ...
        if(numSCompThreads > 1)
        {
            ...
            for (int pos = 0; pos < decompositions.sComponentFSMs.size(); pos++) {
                MultiThreadedConformanceChecker compTask = new MultiThreadedConformanceChecker(pos, decompositions.componentDAFSAs.get(pos), decompositions.sComponentFSMs.get(pos), numTraceThreads);
    ...
    public void recomposeConformance() throws Exception
    {
        ...
        if(conflict > 0)
        {
            ...
            PNMatchInstancesRepResult rescueRes = new MultiThreadedConformanceChecker(new ImportEventLog().createDAFSAfromLog(rescueLog, decompositions.globalInverseLabels),this.decompositions.modelFSM,Integer.MAX_VALUE,numTraceThreads).resOneOptimal;
