...
import au.qut.apromore.psp.Arc;
...
//		this.timeOneOptimal = time.intValue();
    ...
    private void initializePSP()
    {
        ...
        Node source = new Node(logAutomaton.source(), modelStartNodes,
                new Configuration(new IntArrayList(), new IntIntHashMap(), new IntArrayList(), new IntIntHashMap(), new FastList<Couple<Integer, Integer>>(), new FastList<au.qut.apromore.psp.Synchronization>(),
...
//				actMin = potentialFinalNodes.peek().weight();
        ...
        if(!potentialFinalNodes.isEmpty())
        {
            ...
            double moveModel = 0;
            double moveInvi = 0;
            for(au.qut.apromore.psp.Synchronization tr : potentialFinalNode.configuration().sequenceTransitions())
            {
    ...
    private void insertPartiallySynchronizedPathIntoPSP(IntIntHashMap finalConfiguration, IntArrayList traceLabels, Node finalNode)
    {
        ...
        for(au.qut.apromore.psp.Synchronization transition : finalNode.configuration().sequenceTransitions())
        {
                                         ...
                                         IntIntHashMap finalConfiguration, boolean insertToPSP, IntArrayList trace) //, PrintWriter pw)
    {
        ...
        if(operation == Configuration.Operation.MATCH)
        {
            ...
            potentialConfiguration.sequenceTransitions().add(new au.qut.apromore.psp.Synchronization(Configuration.Operation.MATCH, tlog.eventID(), tmodel.eventID(), tlog.target().id(), tmodel.target().id(), currentNode.hashCode()));
        ...
        else if (operation == Configuration.Operation.LHIDE)
        {
            ...
            potentialConfiguration.setMoveOnLog().addToValue(tlog.eventID(), 1);
            //potentialConfiguration.adjustSetMoveOnLog();
            potentialConfiguration.sequenceTransitions().add(new au.qut.apromore.psp.Synchronization(Configuration.Operation.LHIDE, tlog.eventID(), -1, tlog.target().id(), -1, currentNode.hashCode()));
            potentialConfiguration.logIDs().add(tlog.target().id());
            if(insertToPSP)
        ...
        } else
        {
            ...
            potentialConfiguration.setMoveOnModel().addToValue(tmodel.eventID(), 1);
            //potentialConfiguration.adjustSetMoveOnModel();
            potentialConfiguration.sequenceTransitions().add(new au.qut.apromore.psp.Synchronization(Configuration.Operation.RHIDE, -1, tmodel.eventID(), -1, tmodel.target().id(), currentNode.hashCode()));
            potentialConfiguration.modelIDs().add(tmodel.target().id());
            if(insertToPSP)
