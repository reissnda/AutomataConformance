...
public class MultiThreadedTRConformanceChecker implements Callable<MultiThreadedTRConformanceChecker>
{
    ...
    public MultiThreadedTRConformanceChecker(int component, Automaton logAutomatonWithTRReduction, Automaton modelAutomaton, int numThreads)
    {
    ...
    public MultiThreadedTRConformanceChecker(Automaton logAutomatonWithTrReductions, Automaton modelAutomaton, int stateLimit, int numThreads)
    {
        long start = System.currentTimeMillis();
        ...
        if(this.numThreads > 1)
        {
            ...
            for (IntIntHashMap finalConfiguration : logAutomaton.getConfigDecoderMapping().keySet()) {
                for (IntArrayList trace : logAutomaton.getConfigDecoderMapping().get(finalConfiguration)) {
                    CheckTRConformanceCallable confTask = new CheckTRConformanceCallable(psp, logAutomaton, modelAutomaton, finalConfiguration, trace, stateLimit, useVisited);
...
//				System.out.println("long case");
            ...
            if(qStates!=stateLimit)
                if((sizeMoveOnLog=currentNode.configuration().moveOnLog().size())!=decoder.reducedTrace().size())// && visited.size()<=stateLimit)
                {
			...
			*/}
            ...
            if(
                    currentNode.stLog().isFinal()
                            && currentNode.stModel().isFinal()
                            && currentNode.configuration().moveOnLog().equals(decoder.reducedTrace())
                    )
            {
                if(currentNode.weight()<actMin)
                {
...
//				potentialFinalNodes.offer(currentNode);
    ...
    private ReducedResult calculateReducedAlignmentFor(IntIntHashMap finalConfiguration, DecodeTandemRepeats decoder, int stateLimit)
    {
        ...
        while(true)
        {
            ...
            if(currentNode.weight() > actMin) continue;
            numStates++;
            if(useVisited)
                if (visited.containsKey(currentNode.hashCode()))
                {
