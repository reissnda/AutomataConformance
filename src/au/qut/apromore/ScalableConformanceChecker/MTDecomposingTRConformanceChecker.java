...
public class MTDecomposingTRConformanceChecker
{
    private int numTraceThreads;
    private int numScompThreads=1;
    public DecomposingTRImporter decompositions;
    public UnifiedMap<Integer, MultiThreadedTRConformanceChecker> componentAlignments = new UnifiedMap<Integer, MultiThreadedTRConformanceChecker>();
    ...
    public MTDecomposingTRConformanceChecker(DecomposingTRImporter decompositions, int numThreads) throws Exception
    {
    ...
    public MTDecomposingTRConformanceChecker(DecomposingTRImporter decompositions, int numTraceThreads, int numScompThreads) throws Exception
    {
        this.numTraceThreads = numTraceThreads;
        this.numScompThreads = numScompThreads;
        this.decompositions = decompositions;
        checkConformance();
    }

    private void checkConformance() throws Exception
    {
        ...
        else
        {
            ...
            componentAlignments.put(0, new MultiThreadedTRConformanceChecker(decompositions.dafsa, decompositions.modelFSM, Integer.MAX_VALUE, numTraceThreads));
    ...
    public void checkConformanceForComponents() throws Exception
    {
                ...
                compList.parallelStream().forEach
                        (
                                sComp -> componentAlignments.put
                                        (
                                                sComp,
                                                new MultiThreadedTRConformanceChecker(decompositions.componentDAFSAs.get(sComp), decompositions.sComponentFSMs.get(sComp), Integer.MAX_VALUE, numTraceThreads)
                                        )
                        );
        }
        else
        {
            ...
            IntObjectHashMap<Future<MultiThreadedTRConformanceChecker>> futures = new IntObjectHashMap<>();
            for (int pos = 0; pos < decompositions.sComponentFSMs.size(); pos++) {
                MultiThreadedTRConformanceChecker compTask = new MultiThreadedTRConformanceChecker(pos, decompositions.componentDAFSAs.get(pos), decompositions.sComponentFSMs.get(pos), numTraceThreads);
            ...
            for(int pos=0; pos < decompositions.sComponentFSMs.size(); pos++)
            {
                MultiThreadedTRConformanceChecker compResult = null;
                try
                {
        ...
        //this.minModelMoves = ((new SingleTraceConformanceChecker(new IntArrayList(),decompositions.modelFSM,decompositions.globalLabelMapping, decompositions.globalInverseLabels,HashBiMap.create(), new UnifiedMap<>())).res.getInfo().get(PNRepResult.RAWFITNESSCOST));
    ...
    public void recomposeConformance() throws Exception
    {
        ...
        if(conflict > 0)
        {
            ...
            PNMatchInstancesRepResult rescueRes = new MultiThreadedTRConformanceChecker(new ImportEventLog().createReducedDAFSAfromLog(rescueLog, decompositions.globalInverseLabels), decompositions.modelFSM, Integer.MAX_VALUE, numTraceThreads).resOneOptimal();
