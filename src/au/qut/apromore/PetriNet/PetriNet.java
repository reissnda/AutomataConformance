...
public class PetriNet
{
    ...
    public PetriNet(XTrace trace)
    {
        ...
        {
            transitions.add(new Transition(((XAttributeLiteral) trace.get(pos).getAttributes().get(conceptname)).getValue(),true));
            places.add(new Place());
            addPTflow(pos,pos);
            addTPflow(pos,pos+1);
        }
    ...
    {
        if(!isEnabled(transition, marking)) return null;
    ...
    public IntIntHashMap fireUnchecked(int transition, IntIntHashMap marking)
    {
        ...
        for(int key : newMarking.keySet().toArray())
            if(newMarking.get(key)==0) newMarking.remove(key);
        return newMarking;
    }
    ...
    public boolean isEnabled(int transition, IntIntHashMap marking)
    {
        for(int prePlace : tInEdges.get(transition).toArray())
            { if(marking.get(prePlace) < 1) return false; }
        return true;
    }
    ...
    public IntIntHashMap getInitialMarking() { return this.initialMarking; }

    public IntIntHashMap getFinalMarking() { return this.finalMarking; }

    public DoubleMatrix2D getIncidenceMatrix() { return this.incidenceMatrix; }

    public FastList<Place> getPlaces() { return this.places; }

    public FastList<Transition> getTransitions() { return this.transitions; }
