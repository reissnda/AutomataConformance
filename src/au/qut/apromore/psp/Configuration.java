...
public class Configuration 
{
	...
	public enum Operation {MATCH, RHIDE, LHIDE};
	private List<Synchronization> sequenceSynchronizations;
	...
	public Configuration(IntArrayList moveOnLog, IntIntHashMap setMoveOnLog, IntArrayList moveOnModel, IntIntHashMap setMoveOnModel,
			List<Synchronization> sequenceTransitions, IntArrayList logIDs, IntArrayList modelIDs)
	{
	...
	public Configuration(IntArrayList moveOnLog, IntIntHashMap setMoveOnLog, IntArrayList moveOnModel, IntIntHashMap setMoveOnModel, List<Couple<Integer, Integer>> moveMatching,
	...
	public List<Couple<Integer, Integer>> sCompIDs() { return this.sCompIDs; }
...
//	}
	...
	public String printAlignment()
	{
		...
		if(alignment.length()==12) alignment += "]";
		else alignment = alignment.substring(0, alignment.length()-2) +"]";
		return alignment;
	}
	...
	{
		String lastSync = "";
		if(this.sequenceSynchronizations.isEmpty()) lastSync = "[empty]";
		else lastSync = this.sequenceSynchronizations.get(this.sequenceSynchronizations.size()-1).label();
		return lastSync;
	}
...
//		this.adjustSetMoveOnModel();
	...
	*/
    ...
    public boolean equals(Object o) {
        ...
        if(this.sequenceSynchronizations().size() != configuration.sequenceSynchronizations().size()) return false;
        
        for(int i=0; i<this.sequenceSynchronizations().size();i++) {
        	if(this.sequenceSynchronizations().get(i).hashCode() != configuration.sequenceSynchronizations().get(i).hashCode()) return false;
        	if(!this.sequenceSynchronizations().get(i).equals(configuration.sequenceSynchronizations().get(i))) return false;
        }
