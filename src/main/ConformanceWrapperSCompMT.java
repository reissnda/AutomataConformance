...
import au.qut.apromore.ScalableConformanceChecker.MultiThreadedConformanceChecker;
import au.qut.apromore.ScalableConformanceChecker.MultiThreadedDecomposedConformanceChecker;
...
public class ConformanceWrapperSCompMT implements Callable<ConformanceWrapperSCompMT>
{
    ...
    public ConformanceWrapperSCompMT call() throws Exception
    {
        ...
        else if(type.equals("S-Components-with-compare"))
        {
            ...
            for(SyncReplayResult res : repres)
            {
                for(AllSyncReplayResult res2 : pnresult)
                {
                    ...
                    {
                        double difference = Math.round(res2.getInfo().get(PNMatchInstancesRepResult.RAWFITNESSCOST)) - Math.round(res.getInfo().get(PNRepResult.RAWFITNESSCOST));
                        if( difference > 0 )
                        {
        ...
        else if(type.equals("ILP-Alignments"))
        {
            ...
            PNRepResult res = ilp_alignments.computeCost(pnet, xLog,numTraceThreads, true);
            time=TimeUnit.MILLISECONDS.convert(System.nanoTime()-start,TimeUnit.NANOSECONDS);
        ...
        result = type + "," + path +"," + model + "," + nSComps + "," + nParallel +"," + nChoice + "," + log + "," + logSize + "," + time + "," +  cost + "," + conflicts + "\n";
