...
public class BasicCodeSnippet {
	...
	public static void main(String[] args) throws Exception {
		...
		TransEvClassMapping mapping = confChecker.constructMappingBasedOnLabelEquality(net, log, dummyEvClass, eventClassifier);
	...
	public PNRepResult doReplay(XLog log, Petrinet net, Marking initialMarking, Marking finalMarking,
		...
		FastList<Future<TraceReplayTask>> futures = new FastList<Future<TraceReplayTask>>();
		...
		for(int i=0;i<futures.size();i++)
		{
			...
			switch (result.getResult()) {
				...
				case SUCCESS :
					...
					{
						if(replayResult.getStepTypes().get(j)== StepTypes.LMGOOD || replayResult.getStepTypes().get(j)==StepTypes.L)
						{
	...
	public TransEvClassMapping constructMappingBasedOnLabelEquality(PetrinetGraph net, XLog log,
